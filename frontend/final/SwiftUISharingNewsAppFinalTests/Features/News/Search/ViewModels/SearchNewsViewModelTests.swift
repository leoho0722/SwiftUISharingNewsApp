...
struct SearchNewsViewModelTests {
    ...
    @Test("給定有近期搜尋紀錄含關鍵字，當呼叫 applyRecentSearch 時，則 inputKeyword 正確更新")
    func applyRecentSearch_withKeyword_shouldUpdateInputKeyword() {
        // Given
        let mockService = MockNewsService()
        let viewModel = SearchNewsViewModel(newsService: mockService)
        ...
        // When
        viewModel.applyRecentSearch(recentSearch)
        
        // Then
        #expect(viewModel.inputKeyword == "測試關鍵字")
        #expect(viewModel.appliedDateFilter == nil)
    }
    
    @Test("給定有近期搜尋紀錄含日期篩選，當呼叫 applyRecentSearch 時，則 appliedDateFilter 正確更新")
    func applyRecentSearch_withDateFilter_shouldUpdateAppliedDateFilter() {
        // Given
        let mockService = MockNewsService()
        let viewModel = SearchNewsViewModel(newsService: mockService)
        let startDate = Date()
        ...
        #expect(viewModel.inputKeyword.isEmpty)
        #expect(viewModel.appliedDateFilter != nil)
        #expect(viewModel.appliedDateFilter?.startDate == startDate)
        #expect(viewModel.appliedDateFilter?.endDate == endDate)
    }
    ...
    // MARK: - Repository 操作測試
    
    @Test("給定 saveToRecents 為 true，當呼叫 performSearch 時，則 Repository 的 saveSearch 被呼叫")
    func performSearch_withSaveToRecents_shouldCallRepositorySaveSearch() async {
        // Given
        let mockService = MockNewsService()
        mockService.searchNewsResult = .success([])
        let mockRepository = MockRecentSearchRepository()
        let viewModel = SearchNewsViewModel(newsService: mockService)
        viewModel.configure(repository: mockRepository)
        viewModel.inputKeyword = "測試"
        
        // When
        await viewModel.performSearch(saveToRecents: true)
        
        // Then
        #expect(mockRepository.saveSearchCallCount == 1)
        #expect(mockRepository.lastSavedKeyword == "測試")
    }
    
    @Test("給定有近期搜尋紀錄，當呼叫 removeRecentSearch 時，則 Repository 的 deleteSearch 被呼叫")
    func removeRecentSearch_shouldCallRepositoryDeleteSearch() {
        // Given
        let mockService = MockNewsService()
        let mockRepository = MockRecentSearchRepository()
        let viewModel = SearchNewsViewModel(newsService: mockService)
        viewModel.configure(repository: mockRepository)
        let search = RecentSearch(keyword: "測試", startDate: nil, endDate: nil)
        
        // When
        viewModel.removeRecentSearch(search)
        
        // Then
        #expect(mockRepository.deleteSearchCallCount == 1)
    }
    
    @Test("給定有多筆近期搜尋，當呼叫 removeAllRecentSearches 時，則 Repository 的 deleteAllSearches 被呼叫")
    func removeAllRecentSearches_shouldCallRepositoryDeleteAllSearches() {
        // Given
        let mockService = MockNewsService()
        let mockRepository = MockRecentSearchRepository()
        let viewModel = SearchNewsViewModel(newsService: mockService)
        viewModel.configure(repository: mockRepository)
        let searches = [
            RecentSearch(keyword: "測試1", startDate: nil, endDate: nil),
            RecentSearch(keyword: "測試2", startDate: nil, endDate: nil)
        ]
        
        // When
        viewModel.removeAllRecentSearches(searches: searches)
        
        // Then
        #expect(mockRepository.deleteAllSearchesCallCount == 1)
        #expect(mockRepository.lastDeletedSearches?.count == 2)
    }
